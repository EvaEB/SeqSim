...
def select():
    ...
    sim_settings_path = pkg_resources.resource_filename(
        "SeqSimEvo", "simulation_settings/"
    )
    global scenario_settings_path
    scenario_settings_path = pkg_resources.resource_filename(
        "SeqSimEvo", "Scenarios/settings_files/"
    )
...
def settings_press(btn):
    ...
    if btn == "save organism settings":
        ...
        filename = app.saveBox(
            title="Save organism settings", dirName=sim_settings_path, fileExt=""
        )
        # save file
        ...
        # specify file location
        filename = app.saveBox(
            title="Save scenario settings", dirName=scenario_settings_path, fileExt=""
        )
        # save file
        with open(filename, "w") as f:
            f.write(settings["ScenSet"])
    elif btn == "run":
        fasta = app.threadCallback(
            scenarios.run,
            update_fasta,
            major_settings["Scenario"],
        ...
        app.thread(
            fasta_tools.highlighter(fasta_string=settings["fasta_text"], consensus=0)
        )
...
def gui_run():
    ...
    with gui("SeqSim") as app:
        ...
        app.addButtons(
            ["save fasta", "view highlighter", "view nj tree"], settings_press
        )
