        ...
        **kwargs
        ...
        if type(migration) is float or np.shape(migration) == (1, 1):
            ...
            if np.shape(migration) == (n_comparments, n_comparments):
                ...
                except_string = "wrong dimensions of migration matrix. is {}, should be {} or float ".format(
                    np.shape(migration), (n_comparments, n_comparments)
                )
                ...
                if comp1 != comp2:
                    ...
                    for seq in to_migrate:
                        migrants[comp2].append(
                            self.sims[comp1].current_gen.get_seq(seq)
                        )
    ...
    def __str__(self):
        ...
        for i in self.sims:
            out += "compartment {}\t {} seqs\n".format(
                i.settings["name"], i.current_gen.n_seq
            )
        return out
