...
def run(scenario_settings, sim_settings):
    ...
    if type(scenario_settings["sample_sizes"]) is int:
        n_seq = [scenario_settings["sample_sizes"]] * len(
            scenario_settings["sample_times"]
        )
    else:
    ...
    for i in range(scenario_settings["n_gen"]):
        ...
        if i + 1 in scenario_settings["sample_times"]:
            index = [
                j
                for j in range(len(scenario_settings["sample_times"]))
                if scenario_settings["sample_times"][j] == i + 1
            ][0]
            output += sim.current_gen.to_fasta(
                n_seq=n_seq[index], description="-gen{}".format(i + 1)
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="simple simulation of sequence evolution"
    )
    parser.add_argument(
        "-n", nargs=1, type=int, help="number of generations to run the simulation for"
    )
    parser.add_argument(
    ...
    if type(args.st) is not list:
        ...
        settings["sample_times"] = [
            int(i) if i > 0 else int(args.n[0] * -i) for i in args.st
        ]
