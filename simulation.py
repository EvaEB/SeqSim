...
class Simulation(object):
    ...
    def __getfitness_table(self):
        ...
        elif self.settings['model'] == 'exponential': #original model: lethals+beneficials
            for i, j in zip(to_fill[0], to_fill[1]):
                randnr = random.random()
                if randnr > self.settings['parameters']['fl']:
                    if randnr > self.settings['parameters']['fl']+self.settings['parameters']['fb']: #neutral mutation
                        fitness[i, j] = 1
                    else: # beneficial
                        fitness[i, j] = 1+np.random.exponential(self.settings['parameters']['beta'])
...
if __name__ == '__main__':
    times = []
    seq = Seq(seq='A'*5386)
    test = Simulation('phix174',sequence=seq)#,model='neutral', seq_len=500,mutrate=0.1)
    print np.shape(test.fitness_table)
    print sum(sum(test.fitness_table[1:3,:]>1))/(5386.0*3)
